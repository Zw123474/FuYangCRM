<template>
  <div class="app-container">
      ...
      <el-table :data="tableData" stripe style="width: 100%; font-size: 18px" :header-cell-style="{
        ...
        <el-table-column fixed="right" label="操作" width="250" align="center">
          <template slot-scope="scope">
            <el-button type="text" size="small" v-if="isTotalSchedule=='1'" @click="handdleRedistribute(scope.row)">修改重新派发</el-button>
            <el-button type="text" size="small" v-if="isTotalSchedule=='-1' &&isDepartmentHeads=='1'" @click="handdleRevoke(scope.row)">撤销</el-button>
          </template>
        </el-table-column>
      </el-table>
    ...
    <el-dialog title="重新派发部门" :visible.sync="centerDialogVisible" center width="40%">
...
export default {
  ...
  created () {
    // console.log(JSON.parse(localStorage.getItem('userInfo')).isDepartmentHeads);
  ...
  methods: {
    ...
    handdleRedistribute (row) {
      this.centerDialogVisible = true
      this.workId = row.id
    },
    submit(){
      this.checkList = this.handleCheckList(this.checkList)
      let data = {
        id:this.workId,
        departmentIds:this.checkList.toString()
      }
      // console.log(data);
