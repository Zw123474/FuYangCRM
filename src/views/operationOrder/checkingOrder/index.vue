<template>
  <div class="app-container">
      ...
      <el-table :data="tableData" stripe style="width: 100%; font-size: 18px" :header-cell-style="{
        ...
        <el-table-column fixed="right" label="操作" width="250" align="center">
          <template slot-scope="scope">
            <el-button type="text" size="small" @click="bossTurnDown(scope.row)" v-if="isTotalSchedule=='1'">驳回</el-button>
            <el-button type="text" size="small" @click="bossFinish(scope.row)" v-if="isTotalSchedule=='1'">完结</el-button>
            <el-button type="text" size="small" v-if="isOperator=='1'" @click="handdleRevoke(scope.row)">撤销</el-button>
          </template>
        </el-table-column>
      </el-table>
    ...
    <el-dialog title="驳回" :visible.sync="centerDialogVisible" center width="40%">
      <el-form ref="returnForm" :model="addForm" label-width="100px" label-position="top" :rules="returnFormRules">
        <el-row>
          <el-col :span="24">
            <el-form-item label="结单驳回原因" prop="turnDownReason">
              <el-input type="textarea" placeholder="驳回原因" v-model="addForm.turnDownReason" maxlength="300" show-word-limit>
              </el-input>
            </el-form-item>
          </el-col>
        </el-row>
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="submitTurndown">确 定</el-button>
      </span>
    </el-dialog>
    <el-dialog title="完结" :visible.sync="centerDialogVisible2" center width="40%">
      <el-form ref="form" :model="addForm" label-width="100px" label-position="top">
        <el-row>
          <el-col :span="24" v-if="workOrderType == 'SPECIAL'">
            <el-form-item label="用户编号" required>
              <el-select v-model="addForm2.userNumber" placeholder="用户编号" filterable>
                <el-option v-for="(item,inde) in oneToOneUserOptions" :key="inde" :label="item.name" :value="item.name">
                </el-option>
              </el-select>
            </el-form-item>
          ...
          <el-col :span="24">
            <el-form-item label="审核备注">
              <el-input type="textarea" placeholder="审核备注" v-model="addForm2.reviewNotes" maxlength="300" show-word-limit>
              </el-input>
            </el-form-item>
          </el-col>
        </el-row>
...
export default {
  ...
  methods: {
    ...
    bossFinish (row) {
      ...
      this.orderId = row.id
      this.workOrderType = row.workOrderType
    },
    submitFinish () {
      let data = {
        id: this.orderId,
        reviewNotes: this.addForm2.reviewNotes,
        userNumber: this.addForm2.userNumber
      }
    ...
    // checkingDetails (row) {
    //   this.$router.push({ name: "checkingDetails", query: { id: row.id } });
    // },
    resetSearch () {
      this.search = {
        createTime: "",
    ...
    getOrderList () {
      let data = {
        ...
        repairCategory: this.search.repairCategory,
